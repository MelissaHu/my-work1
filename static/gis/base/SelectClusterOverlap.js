 ...
 * @api stable
 */
var ol_interaction_SelectCluster = function (options) {	options = options || {}
  this.maxSelect = options.maxSelect || 20
  this.pointRadius = options.pointRadius || 12
	this.circleMaxObjects = options.circleMaxObjects || 20
	...
	if (options.layers)
  {
	if (typeof (options.layers) === 'function') {	var fn = options.layers
...
ol_interaction_Select.prototype.setMap.call(this, map)
	...
	if (map && map.getView()) {	map.getView().on('change:resolution', this.clear, this)
	}
}
 ...
 * @api stable
 */
ol_interaction_SelectCluster.prototype.clear = function () {	this.getFeatures().clear()
 ...
 * @api stable
 */
ol_interaction_SelectCluster.prototype.selectCluster = function (e) {	// Nothing selected
  ...
  // Draw on a spiral
    ...
    var a = 0
		...
		// Feature on a spiral
    if (this.maxObjects < this.maxSelect) { // 如果聚合数大于maxSelect则不能点开
      for (var i = 0; i < max; i++) {	// New radius => increase d in one turn
        r = d / 2 + d * a / (2 * Math.PI)
			// Angle
			a = a + (d + 0.1) / r
 ...
 */
ol_interaction_SelectCluster.prototype.animateCluster_ = function (center) {	// Stop animation (if one is running)
  if (this.listenerKey_) {	this.overlayLayer_.setVisible(true)
  ...
  var duration = this.animationDuration || 500
	...
	// Animate function
	function animate (event) {	var vectorContext = event.vectorContext
		// Retina device
    ...
    if (e > 1.0) {	ol_Observable.unByKey(this.listenerKey_)
